...
public class MapView extends MasterViewPanel {
	 ...
	 * @author AJ Venne
	 */
	private static final long serialVersionUID = 5977780590454654721L;
	...
	private void setUpGUI() {
		...
		// NOTE
		// Currently a bug that needs to be worked out: Buffered Image and
		// displayed image are not in exact spot when this is added, hard to
		// track since
	...
	public void paintComponent(Graphics g) {
		...
		// mapImage = mapImage.getScaledInstance(this.getWidth(),
		// this.getHeight(), 2);
	...
	private class mouse implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			if ((e.getX() < buffImage.getWidth())
					&& (e.getY() < buffImage.getHeight()))
				System.out.println(territories.get(
						getLocation(e.getX(), e.getY())).toString());
		}
	...
	private class mouseMove implements MouseMotionListener {
		...
		public void mouseMoved(MouseEvent arg0) {
			// checking which territory, for use with tool tip which will later
			// display owner, etc. info
			// NORTH AMERICA
			if ((arg0.getX() < buffImage.getWidth())
					&& (arg0.getY() < buffImage.getHeight()))
	...
	private String getLocation(int x, int y) {
		...
		// NORTH AMERICA
		// Area 1
		if ((buffImage.getRGB(x, y) == -8355840)
				&& (y < (buffImage.getHeight() / 5.3125)))
			return "Alaska"; // + na.get("Alaska").getOwner().getTeam();
		// Area 2
		if ((buffImage.getRGB(x, y) == -256)
				&& (x < (buffImage.getWidth() / 4.066666667)))
			return "Alberta";
		// Area 3
		if ((buffImage.getRGB(x, y) == -128)
				&& (y > (buffImage.getHeight() / 3.4)))
		...
		if ((buffImage.getRGB(x, y) == -256)
			...
			return "Greenland";
		// Area 6
		if ((buffImage.getRGB(x, y) == -11513817)
				&& (y < (buffImage.getHeight() / 5.3125)))
		...
		// AFRICA
		// Area 1
		if ((buffImage.getRGB(x, y) == -5351680)
				&& (x < (buffImage.getWidth() / 1.694444444)))
			return "Congo";
		// Area 2
		if (buffImage.getRGB(x, y) == -32768)
			return "East Africa";
		// Area 3
		if ((buffImage.getRGB(x, y) == -8372224)
				&& (y < (buffImage.getHeight() / 2.266666667)))
			return "Egypt";
		// Area 4
		if ((buffImage.getRGB(x, y) == -5351680)
				&& (x > (buffImage.getWidth() / 1.694444444)))
		...
		// EUROPE
		// Area 1
		if ((buffImage.getRGB(x, y) == -16760704)
				&& (y < (buffImage.getHeight() / 4.857142857)))
			return "Great Britain";
		// Area 2
		if ((buffImage.getRGB(x, y) == -16776961)
				&& (y < (buffImage.getHeight() / 7.555555556)))
			return "Iceland";
		// Area 3
		if ((buffImage.getRGB(x, y) == -16776961)
				&& (y > (buffImage.getHeight() / 7.555555556)))
		...
		// Area 4
		if ((buffImage.getRGB(x, y) == -16744193)
				&& (y < (buffImage.getHeight() / 5.666666667)))
			return "Scandinavia";
		// Area 5
		if ((buffImage.getRGB(x, y) == -16760704)
				&& (y > (buffImage.getHeight() / 4.857142857)))
		...
		// ASIA
		// Area 1
		if ((buffImage.getRGB(x, y) == -8323200)
				&& (x < (buffImage.getWidth() / 1.355555556)))
			return "Afghanistan";
		// Area 2
		if ((buffImage.getRGB(x, y) == -16744384)
				&& (y > (buffImage.getHeight() / 4.4)))
			return "China";
		// Area 3
		if ((buffImage.getRGB(x, y) == -16744320)
				&& (y > (buffImage.getHeight() / 4.689655172)))
			return "India";
		// Area 4
		if ((buffImage.getRGB(x, y) == -8323328)
				&& (x < (buffImage.getWidth() / 1.167464115)))
			return "Irkutsk";
		// Area 5
		if ((buffImage.getRGB(x, y) == -8323328)
				&& (x > (buffImage.getWidth() / 1.167464115)))
		...
		// Area 6
		if ((buffImage.getRGB(x, y) == -16744384)
				&& (y < (buffImage.getHeight() / 4.657534247)))
			return "Kamchatka";
		// Area 7
		if ((buffImage.getRGB(x, y) == -16744448)
				&& (y > (buffImage.getHeight() / 4)))
			return "Middle East";
		// Area 8
		if ((buffImage.getRGB(x, y) == -16760832)
				&& (x > (buffImage.getWidth() / 1.384790011)))
			return "Mongolia";
		// Area 9
		if ((buffImage.getRGB(x, y) == -8323200)
				&& (x > (buffImage.getWidth() / 1.355555556)))
